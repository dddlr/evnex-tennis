...
describe('Tennis match', () => {
    ...
    describe('Whole match', () => {
        ...
        it('should work for a typical match (player 1 wins, 7-5)', () => {
            for (let i = 0; i < 5*GAME_WINNING_POINTS; i++) match.pointWonBy('player 1');

            assert.equal(match.score(), '5-0');

            for (let i = 0; i < 5*GAME_WINNING_POINTS; i++) match.pointWonBy('player 2');

            assert.equal(match.score(), '5-5');

            for (let i = 0; i < GAME_WINNING_POINTS; i++) match.pointWonBy('player 1');
        ...
        it('should work for a tie-break match (6-6)', () => {
            ...
            for (let i = 0; i < GAME_WINNING_POINTS; i++) match.pointWonBy('player 2');
